...
namespace HardCodeLab.TutorialMaster.EditorUI
{
    ...
    public sealed class TutorialMasterEditor : Editor
    {
        ...
        {
            if (_debugControllerEditor == null) _debugControllerEditor = new DebugControllerEditor(ref _editor);
            if (_localizationSettings == null) _localizationSettings = new LocalizationEditor(ref _editor);
            if (_startupSettings == null) _startupSettings = new StartupSettingsEditor(ref _editor);
            if (_poolingEditor == null) _poolingEditor = new PoolingEditor(ref _editor);
            if (_tutorialList == null) _tutorialList = new TutorialListEditor(ref _editor);
            if (_tutorialSettings == null) _tutorialSettings = new TutorialEditor(ref _editor);
            if (_stageList == null) _stageList = new StageListEditor(ref _editor);
            if (_stageSettings == null) _stageSettings = new StageEditor(ref _editor);
        }

        /// <summary>
        /// Loads an appropriate GUI skin for the Editor.
        /// </summary>
        private void LoadGUISkin()
        {
            if (EditorGUIUtility.isProSkin)
            {
                GUI.skin = (GUISkin)AssetDatabase.LoadAssetAtPath(
                    TMEditorUtils.DirectoryPath + "/Skins/TM_Skin_Dark.guiskin",
                    typeof(GUISkin));
            }
            else
            {
                GUI.skin = (GUISkin)AssetDatabase.LoadAssetAtPath(
                    TMEditorUtils.DirectoryPath + "/Skins/TM_Skin_Light.guiskin",
                    typeof(GUISkin));
            }

            if (_box == null) _box = new HCLBox(TMEditorUtils.DirectoryPath + "/Skins/Items/HCLBox/");
        }
        ...
        {
            Repaint();
            _debugControllerEditor.RenderGUI(_box, canRender: EditorApplication.isPlaying);
            ...
            _tutorialList.RenderGUI(
                ...
                brief: SelectedTutorial != null
                   ...
                   : "");
            _tutorialSettings.RenderGUI(_box, canRender: SelectedTutorial != null);
            ...
            _stageSettings.RenderGUI(_box, canRender: SelectedTutorial != null && SelectedStage != null);
        }
    }
}